    ...
    public function loginMe()
    {
        ...
        else
        {
            ...
            if(!empty($result))
            {
                ...
                $sessionArray = array('userId'=>$result[0]['login_id'],
                                        // 'role'=>$result->roleId,
                                        // 'roleText'=>$result->role,
                                        'name'=>$result[0]['username'],
                                        // 'isAdmin'=>$result->isAdmin,
                                        // 'accessInfo'=>$accessInfo,
                                        'lastLoginIp'=> $result[0]['last_login_ip'],
                                        'isLoggedIn' => TRUE
                                );

                $this->session->set_userdata($sessionArray);

                unset($sessionArray['userId'], $sessionArray['lastLoginIp']);

                $loginInfo = array("userId"=>$result->login_id, "sessionData" => json_encode($sessionArray), "machineIp"=>$_SERVER['REMOTE_ADDR'], "userAgent"=>getBrowserAgent(), "agentString"=>$this->agent->agent_string(), "platform"=>$this->agent->platform());
